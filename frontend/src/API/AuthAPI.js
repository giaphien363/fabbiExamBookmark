...
const LoginGoogle = async(accesstoken) => {
    let res = await axios.post("http://localhost:8000/social-login/google/", {
...
const LoginFacebook = async(accesstoken) => {
    let res = await axios.post("http://localhost:8000/social-login/facebook/", {
        access_token: accesstoken,
    });
    return await res;
};

export { Refresh, LoginAPI, Register, LoginGoogle, LoginFacebook };